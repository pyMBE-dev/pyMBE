...
class Test(ut.TestCase):
    ...
    def test_salt_addition(self):
        """
        Unit test to check that create_added_salt works for different types of salts.
        """
        def check_salt_concentration(espresso_system,cation_name,anion_name,c_salt,N_SALT_ION_PAIRS):
            ...
            for id in cation_ids+anion_ids:
                pmb.delete_instances_in_system(instance_id=id,
                                                espresso_system=espresso_system,
                                                pmb_type="particle")

        # Unit test: test that create_added_salt works for a 1:1 salt (NaCl-like). 
        check_salt_concentration(espresso_system=espresso_system,
                                cation_name="Na",
                                anion_name="Cl",
                                c_salt=c_salt_input,
                                N_SALT_ION_PAIRS=N_SALT_ION_PAIRS)
        # test that create_added_salt works for a 2:1 salt (CaCl_2-like)
        check_salt_concentration(espresso_system=espresso_system,
                                cation_name="Ca",
                                anion_name="Cl",
                                c_salt=c_salt_input,
                                N_SALT_ION_PAIRS=N_SALT_ION_PAIRS)
        # test that create_added_salt works for a 1:2 salt (Na_2SO_4-like)
        check_salt_concentration(espresso_system=espresso_system,
                                cation_name="Na",
                                anion_name="SO4",
                                c_salt=c_salt_input,
                                N_SALT_ION_PAIRS=N_SALT_ION_PAIRS)
        # test that create_added_salt works for a 2:2 salt (CaSO_4-like)
        check_salt_concentration(espresso_system=espresso_system,
                                cation_name="Ca",
                                anion_name="SO4",
                                c_salt=c_salt_input,
    ...
    def test_salt_addition_concentration_units(self):
        """
    ...
    def test_sanity_create_salt(self):
        """
        Unit tests to check that create_added_salt raises ValueErrors when expected.
        """

        #check that create_added_salt raises a ValueError if one provides a cation_name of an object that has been defined with a non-positive charge
        input_parameters={"cation_name":"Cl",
                            "anion_name":"SO4",
                            "c_salt":c_salt_input,
                        "espresso_system":espresso_system}
        self.assertRaises(ValueError, pmb.create_added_salt, **input_parameters)
        # check that create_added_salt raises a ValueError if one provides a anion_name of an object that has been defined with a non-negative charge 
        input_parameters={"cation_name":"Na",
                            "anion_name":"Ca",
                            "c_salt":c_salt_input,
                        "espresso_system":espresso_system}
        self.assertRaises(ValueError, pmb.create_added_salt, **input_parameters)
        # check that create_added_salt raises a ValueError if one provides a c_salt with the wrong dimensionality 
        input_parameters={"cation_name":"Na",
                            "anion_name":"Cl",
        ...
        input_parameters={"cation_name":"Na",
                        ...
                        "espresso_system":espresso_system}
        self.assertRaises(ValueError, pmb.create_added_salt, **input_parameters)
        input_parameters={"cation_name":"X",
                            "anion_name":"Cl",
                            "c_salt":c_salt_input,
                        "espresso_system":espresso_system}
        self.assertRaises(ValueError, pmb.create_added_salt, **input_parameters)
    
    def test_counterions_setup(self):
        """
        Unit test to check that create_counterions sets up correctly counterions for charged polyampholytes.
        """
        ...
        # Check that create_counterions creates the right number of monovalent counter ions for a polyampholyte with positive net charge. 
        test_counterions(molecule_name='positive_polyampholyte', 
                        ...
                        expected_numbers={"Na":4,
                                        "Cl":6})
        # Check that create_counterions creates the right number of monovalent counter ions for a polyampholyte at its isoelectric point
        test_counterions(molecule_name='isoelectric_polyampholyte', 
                        ...
                        expected_numbers={"Na":6,
                                        "Cl":6})
        # Check that create_counterions creates the right number of monovalent counter ions for a polyampholyte with a negative net charge
        test_counterions(molecule_name='negative_polyampholyte', 
                        ...
                        expected_numbers={"Na":6,
                                        "Cl":4})
        # Check that create_counterions creates the right number of multivalent counter ions for a polyampholyte
        test_counterions(molecule_name='negative_polyampholyte', 
                        cation_name="Ca", 
                        anion_name="Cl", 
                        espresso_system=espresso_system, 
                        expected_numbers={"Ca":3,
                                        "Cl":4})
    def test_sanity_create_counterions(self):
        """
        Unit tests to check that create_counterions raises ValueErrors when expected.
        """
        # Check that create_counterions raises a ValueError if the charge number of the cation is not divisible by the negative charge of the polyampholyte
        ...
        input_parameters={"cation_name":"Ca",
                        ...
                        "espresso_system":espresso_system}
        self.assertRaises(ValueError, pmb.create_counterions, **input_parameters)
        # Check that create_counterions raises a ValueError if the charge number of the anion is not divisible by the positive charge of the polyampholyte
        input_parameters={"cation_name":"Na",
                            "anion_name":"SO4",
                            "object_name":'isoelectric_polyampholyte',
                        "espresso_system":espresso_system}
        ...
        pmb.delete_instances_in_system(instance_id=0,
                                    espresso_system=espresso_system,
                                    pmb_type="molecule")
        # Check that no create_counterions does not create counterions for molecules with no charge       
